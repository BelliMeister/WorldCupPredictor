...
from features import FEATURE_COLS, ELO_DEFAULT, load_fifa_rankings, build_rank_lookup, get_rank_at
...
def build_match_features(home, away, elo_ratings, rank_lookup, is_neutral=True):
    ...
    features = [
        ...
        D_SCORED, D_CONCEDED, D_WIN, D_PTS,
        D_SCORED, D_CONCEDED, D_WIN, D_PTS,
        0.5, 2.5,
        rank_diff, pts_diff, home_rank, away_rank,
        int(is_neutral), 1,
    ]
...
# ── Discipline & set pieces from master.csv ────────────────────────────────────
...
def find_value_bets(probs, odds, min_edge=0.03):
    bets = []
...
def print_discipline(home: str, away: str):
    ...
    print("\n  DISCIPLINE & SET PIECES  (avg per game, recent WC/qualifier matches)")
    print(f"  {'':28}  {'Corners':>8}  {'Yellow':>8}  {'Red':>6}")

    def row(name, stats):
        if not stats:
            print(f"  {name:<28}  {'N/A':>8}  {'N/A':>8}  {'N/A':>6}")
            return
        corners = f"{stats['corners']:.1f}" if stats.get("corners") is not None else "N/A"
        yellow  = f"{stats['yellow']:.1f}"  if stats.get("yellow")  is not None else "N/A"
        red     = f"{stats['red']:.1f}"     if stats.get("red")     is not None else "N/A"
        print(f"  {name:<28}  {corners:>8}  {yellow:>8}  {red:>6}")
...
def main():
    ...
    # Split predicted total into home/away using 55/45 home-attack ratio as neutral-venue default
    home_mu = predicted_total * 0.50
    away_mu = predicted_total * 0.50
    ...
    if has_odds:
        ...
        if bets:
            ...
            print("\n  No value found — book has edge on all outcomes at this line.")
    else:
